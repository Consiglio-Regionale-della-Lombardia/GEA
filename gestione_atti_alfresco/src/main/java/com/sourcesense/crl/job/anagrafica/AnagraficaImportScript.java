...
import com.sourcesense.crl.util.CommissioneUtil;
...
public class AnagraficaImportScript extends BaseScopableProcessorExtension {
    ...
    private CommissioneUtil commissioneUtil;
    ...
    public void executeImport() {
        ...
        String nomeLegislaturaCorrente;
        ...
        try {
            ...
            if (currentlegislatureNodeResultSet.length() == 1) {
                ...
                nomeLegislaturaCorrente = (String) nodeService.getProperty(currentlegislatureNodeResultSet.getNodeRef(0), ContentModel.PROP_NAME);
                ...
                for (Councilor councilor : councilors) {
                    ...
                    if (councilor.getCommittees().size() == 0) {
                        logger.warn("Non sono stati trovati committes nel counsilor " + councilor.getFirstName() + " " + councilor.getLastName());
                    }
                    ...
                    for (int k = 0; k < councilor.getCommittees().size(); k++) {
                        ...
                        int identificatoreCommissione = councilor.getCommittees().get(k).getId();
                        boolean deleted = councilor.getCommittees().get(k).isDeleted();
                        String idCommissione = commissioneUtil.generaIdCommissione(committeName, nomeLegislaturaCorrente, committeOrder, identificatoreCommissione);
                        ...
                        if (!committesMap.containsKey(committeName)) {
                            ...
                            String luceneQueryCommissione = "PATH:\"/app:company_home/cm:CRL/cm:Gestione_x0020_Atti/cm:Anagrafica/cm:Commissioni/cm:" + ISO9075.encode(idCommissione) + "\"";
                            ...
                            if (commissioneResults != null && commissioneResults.length() > 0) {
                                ...
                                commissioneCorrente = fileFolderService.create(committesFolderNodeResultSet.getNodeRef(0), idCommissione, Constant.TYPE_COMMISSIONE_ANAGRAFICA).getNodeRef();

                            }
                            nodeService.setProperty(commissioneCorrente, Constant.PROP_NUMERO_ORDINAMENTO_COMMISSIONE_ANAGRAFICA, committeOrder);
                            nodeService.setProperty(commissioneCorrente, Constant.PROP_IDENTIFICATORE_COMMISSIONE_ANAGRAFICA, identificatoreCommissione);
                            nodeService.setProperty(commissioneCorrente, ContentModel.PROP_TITLE, committeName);
                            ...
                            //Il nuovo nome del gruppo non avra il nome della commissione ma l'id della commissione
                            String alfrescoGroupName = nomeLegislaturaCorrente + "_" + idCommissione;

                            // Creo il GROUP di commissione solo se non è già censito a sistema
                            if (authorityService.getAuthorityNodeRef("GROUP_" + alfrescoGroupName) == null) {
                                ...
                                authorityService.createAuthority(AuthorityType.GROUP, alfrescoGroupName, committeName, authorityService.getDefaultZones());
                                authorityService.addAuthority(Constant.GROUP_COMMISSIONI, "GROUP_" + alfrescoGroupName);
                                logger.info("Creata l'authority '" + alfrescoGroupName + "' con authorityDisplayName '" + committeName + "'");
                            }
                        ...
                        createCouncilorOnCommitte(councilor, committesMap.get(committeName), nomeLegislaturaCorrente);
    ...
    /**
     * Crea un cosigliere inserendolo nella cartella folderNodeRef, che è la cartella della commissione.
     *
     ...
     */
    private NodeRef createCouncilorOnCommitte(Councilor councilor, NodeRef folderNodeRef, String nomeLegislaturaCorrente) {
        ...
        //Il nuovo nome del folder commissione anagrafica è il ID della commissione, non il nome della commissione
        String nomeFolderNode = (String) nodeService.getProperty(folderNodeRef, Constant.PROP_IDENTIFICATORE_COMMISSIONE_ANAGRAFICA);
    ...
    /**
     * Crea un cosigliere inserendolo nella cartella folderNodeRef, che è la cartella della commissione quando non è un consigliere storico.
     *
     * @param councilor               Struttura di dati con tutti i dati anagrafici del consigliere
     * @param folderNodeRef           Cartella dove verrà creato il consigliere SE NON ESISTE ANCORA
     * @param nomeLegislaturaCorrente idem
    ...
    public CommissioneUtil getCommissioneUtil() {
        return commissioneUtil;
    }


    public void setCommissioneUtil(CommissioneUtil commissioneUtil) {
        this.commissioneUtil = commissioneUtil;
    }
